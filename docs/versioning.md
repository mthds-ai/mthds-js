...
An envelope raised **before** the read carries no `skipped_methods` at all, because there is no skip list yet, and that absence is the signal a consumer reads. Keeping the two apart is why a pure argument check belongs with the other argument checks at the top of a command rather than after the resolve — `mthds-agent share` validates `--platform` up front for that reason, and rejects a misspelled platform without fetching anything.
