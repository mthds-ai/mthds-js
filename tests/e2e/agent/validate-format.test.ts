...
describe("mthds-agent validate --format/--error-format value validation (e2e)", () => {
  it("rejects an invalid --format value instead of silently coercing to markdown", () => {
    const { stderr, status } = runAgent("--runner", "api", "validate", "bundle", "nonexistent.mthds", "--format", "jsno");
    expect(status).toBe(1);
    const payload = JSON.parse(stderr) as { error?: boolean; error_type?: string; message?: string };
  ...
  it("rejects an invalid --error-format value on validate pipe", () => {
    const { stderr, status } = runAgent("--runner", "api", "validate", "pipe", "nonexistent.mthds", "--error-format", "yaml");
    expect(status).toBe(1);
