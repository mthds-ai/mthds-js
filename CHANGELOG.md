...
- **A CI gate on the shared protocol fixture corpus.** `tests/fixtures/protocol/` is committed byte-identically here and in `mthds-python`, and until now nothing in either repo checked that: the only comparison lived in `conformance`, where it ran on a `conformance` pull request rather than on the one that moved the corpus. A recapture landed here without its `mthds-python` twin was therefore caught weeks later by an unrelated change, and the pairing held only because prose in two pull request bodies asked for it. `.github/workflows/protocol-corpus-parity.yml` now checks `mthds-python` out on every pull request and compares the two trees, so a one-sided recapture fails the pull request that causes it.
...
- **The inputs-template projection.** `src/protocol/inputs_template.ts` projects one pipe's `input_form` descriptor into the fill-in inputs template a client used to fetch from `POST /v1/build/inputs` — both shapes (the compact form a smart-inputs run accepts directly, and the explicit `{concept, content}` envelope) in both formats (JSON and TOML) — so a client holding a descriptor can offer a template for a method it does not have on disk. `renderInputsTemplate` is the entry point, with `projectInputsTemplate`, `projectConceptComments` and `formatSlotSignature` beside it, all on the `mthds/protocol` subpath. It is the TypeScript half of a pair: `mthds-python` carries the twin, and the shared fixture corpus holds the two to the same bytes, TOML `# concept:` comment lines included. Two pieces exist because of that bar. `src/protocol/toml_emitter.ts` states the TOML layout in full rather than delegating to `smol-toml` — a library emits no comments and may change its layout in a patch release, in one language and not the other — and a `TemplateFloat` marker carries the decimal point that TypeScript's single number type would otherwise drop, which is also why the JSON is written here rather than handed to `JSON.stringify`. The projection walks the descriptor and never a runtime content class, so an `enum` takes its first choice, an `unknown` node renders as an empty mapping, and a fixed `Concept[N]` slot renders `N` distinct elements. Each rule the captured bundles cannot reach is stated on its own: `tests/unit/protocol/toml-emitter.test.ts` holds every layout rule as bytes and refuses a comment that would not stay one line, and `inputs-template-rendering.test.ts` covers an input-less pipe, a plural native slot and an unknown format.

- **The shared projection fixture corpus.** `tests/fixtures/protocol/inputs_template/` pairs each captured pipe's input-form descriptor with the fill-in inputs template a client-side projection must produce from it — the compact and explicit shapes, as JSON and as TOML — and `tests/unit/protocol/inputs-template-projection.test.ts` holds the corpus to five properties: the whole closed `FIELD_KINDS` vocabulary is exercised, every pipe is present in both shapes and both formats, each deliberate departure from the reference engine's own renderer is still visible in the committed bytes, the manifest's `unshapeable` record stays keyed to this corpus, and the projection reproduces every file byte for byte. That `unshapeable` array is the capture's own statement about which of its pinned templates the runtime's input shaper refuses today: the generator round-trips every projected template through `InputShaper.shape` and records each refusal with the error's class name and the ledger item whose fix retires it, so a template that cannot be filled in and handed back is declared rather than discovered. The entries this capture carries are the four templates blocked on the nested-list slot the shaper cannot take back. The corpus is committed identically in `mthds-python`, which runs the twin of that suite, and `conformance` compares the two mirrors file by file — that byte identity is the point, since the two projections are written in different languages and must agree exactly, TOML comment lines included. The descriptor and contract captures are regenerated from the same command and gain a third bundle covering a text field merely named `url` beside a real file position, optional file fields nested inside a structure, and both a fixed `[N]` and a variable `[]` slot over a structured concept.

### Changed

- **The projection fixture corpus covers the natives whose payload shape the descriptor states openly.** A new capture from `pipelex` `dev` adds `input_semantics_scaffold.scaffold_open_natives`, a pipe putting `native.JSON`, `native.Dynamic` and `native.Composite` each at a slot position — the one place the corpus never reached. `Dynamic` and `Composite` are structureless by design, so their node is `unknown` and both renderers agree on the empty mapping; `JSON` is not, because its pinned blueprint carries a required `json_obj`, which puts an unknown node at a **required object-field** position for the first time in the corpus — the required unknown nodes it already held sit at list-item positions, where both renderers agree. That position is what the capture declares as a new divergence class, `unknown-empty-object`: the projection renders the empty object, because the descriptor withholds the payload shape there and inventing one would stop it projecting the descriptor, while the reference engine reflects the runtime content class and fills the dict with a sample key/value pair whoever fills the template in has to delete. The empty object round-trips through the input shaper cleanly, so the class records a difference of vantage rather than a defect and carries no ledger item. `input_form.json`, `pipe_io_contracts.json` and `inputs_template/manifest.json` move with it, the `*.fixture.ts` twins are regenerated (still with no known divergence to suppress), and `mthds-python` commits the identical bytes. The README's prose list of divergence classes is now held to the manifest's own declarations by `inputs-template-projection.test.ts`, so a class retired upstream cannot leave its bullet behind describing a difference that no longer exists.

- **`OutputForm` and `PipeOutputFormDescriptor` (`protocol/output_form.ts`) — the output-form descriptor, mirroring the standard's new page and `mthds-python` exactly.** A descriptor carries one `field`, not a `fields` list: a pipe has exactly one output where it may have many inputs, which is the only shape difference from its input twin and follows from the language rather than from taste. `OutputForm` is the `pipe_ref` → descriptor map, over the same key set as `PipeIOContracts` and `InputForm`.

  The module declares no node type. `field` is `InputFormField`, imported verbatim — an output is a concept reference exactly like an input is, so its kinds, its nesting and its constraints are the same questions with the same answers, and `FIELD_KINDS` covers this artifact too. What an output does NOT carry is expressed the way this package expresses "does not apply": the node simply never gets `presence` or `gating`, and `InputFormTopLevelField` — the layer that adds them — is not used. Both are facts of an input slot: `!` may not appear on an output, `?` is what the contract's `optional` states, and nothing waits on a result.

- **`json_schema` on `PipeOutputContract` — the payload's schema (breaking).** The output contract carried no schema, on the reasoning that a run's payload is the run's own result. That answers *what did this run produce?*, correctly not a contract's business, rather than the question a consumer asks: *what shape will it be?* — declared in the source, knowable before any run.
  ...
  **It is the PAYLOAD's schema, and that is where its rule departs from the input twin's.** An input's describes what a caller *sends*, so a plural slot's is a bare array. An output's describes what *comes back*, which is the concept's content model — so a `native.Text` output's is the object declaring `text`, and a `Concept[]` output's is the list content model rather than a bare array. On the fixed arm the element array carries `minItems`/`maxItems` equal to `item_count`; since `optional` already states the output may be absent, the schema describes the shape when present and carries no null arm.
  ...
  Breaking for any hand-built `PipeIOContract` literal: the member is required, so a literal without it no longer type-checks. The one negative case in the shape tests that pinned "the output contract carries no schema" is inverted — it now pins `presence` on an output, which is the member the shape most plausibly grows by mistake.

- **`output_form.json` and its generated twin**, captured from the same engine run as its two siblings and committed byte-identically in `mthds-python`. `input-form-parity.test.ts` gains the cross-artifact rules a type cannot express: all three artifacts keyed alike, no slot fact on an output node, the **plural wrap** (a plural output described as a `list`, a single one not — the one producer obligation that fails silently), the fixed count agreeing between descriptor and contract, and the output schema being a content model rather than a bare array.

- **The shared projection fixture corpus.** `tests/fixtures/protocol/inputs_template/` pairs each captured pipe's input-form descriptor with the fill-in inputs template a client-side projection must produce from it — the compact and explicit shapes, as JSON and as TOML — and `tests/unit/protocol/inputs-template-projection.test.ts` holds the corpus to five properties: the whole closed `FIELD_KINDS` vocabulary is exercised, every pipe is present in both shapes and both formats, each deliberate departure from the reference engine's own renderer is still visible in the committed bytes, the manifest's `unshapeable` record stays keyed to this corpus, and (skip-gated until the projection itself lands) the projection reproduces every file byte for byte. That `unshapeable` array is the capture's own statement about which of its pinned templates the runtime's input shaper refuses today: the generator round-trips every projected template through `InputShaper.shape` and records each refusal with the error's class name and the ledger item whose fix retires it, so a template that cannot be filled in and handed back is declared rather than discovered. The entries this capture carries are the four templates blocked on the nested-list slot the shaper cannot take back. The corpus is committed identically in `mthds-python`, which runs the twin of that suite, and `conformance` compares the two mirrors file by file — that byte identity is the point, since the two projections are written in different languages and must agree exactly, TOML comment lines included. The descriptor and contract captures are regenerated from the same command and gain a third bundle covering a text field merely named `url` beside a real file position, optional file fields nested inside a structure, and both a fixed `[N]` and a variable `[]` slot over a structured concept, plus a fourth covering OUTPUTS — flat and nested structures, both plural arms, an optional output, a wrapping native scalar and a file-bearing one. That fourth bundle exists because the three before it all resolved to `Text`: a corpus whose every pipe outputs one native scalar can only capture one output descriptor and one output schema, and the two facts most likely to be got wrong would never appear in it.

### Fixed

- **The build envelope no longer says `method_ref` is unusable.** `BuildRequestBase`, the local runner's rejection message, and `docs/build-routes.md` all claimed the server answers `501` for any `method_ref` because no method registry exists. That has not been true since pipelex-api 0.21.0, which resolves the **address form** (`github.com/<owner>/<repo>[/<selector>][@<tag>]`) on the build routes — fetching the repository at the tag, locating the package by manifest identity, and feeding the closure with the package's real relative paths. Only the **registry form** still answers `501`. An omitted `pipe_ref` defaults to the fetched manifest's `main_pipe` only on a server newer than 0.21.0 — 0.21.0 itself drops the manifest on the tooling path and falls through to the closure's own declarations — so the envelope documents both arms rather than promising the manifest default outright. The local runner keeps refusing both forms, because it hands a closure already on disk to `pipelex-agent <projection> bundle <path>` and has nothing to resolve a reference against, but it now says so as a fact about itself and points at the API runner instead.
...
- **Raised the runtime version floors `mthds-agent` enforces.** `PIPELEX_PKG.version_constraint` moves to `>=0.46.4` (the `pipelex` PyPI package, covering both the `pipelex` and `pipelex-agent` binaries), `PIPELEX_TOOLS_PKG.version_constraint` to `>=0.8.0` (the `pipelex-tools` package, covering the `plxt` binary), `MIN_PLUGIN_VERSION` to `>=0.15.1` (the Claude Code mthds plugin), and `MIN_CODEX_VERSION` to `0.144.0` (the Codex app, checked best-effort by `codex apply-config` and `doctor`). Below its floor each surface reports the way it did before — an install/upgrade recovery hint for the uv-installable packages, a `CODEX_VERSION_TOO_OLD` warning for Codex. `CODEX_PLUGIN_HOOKS_MIN` (the fixed `0.131.0` boundary below which plugin-bundled hooks cannot load) is unchanged.
...
- **Config-dialect conformance fixture:** Added a shared fixture (`tests/fixtures/config-dialect-cases.json`), pinned by the workspace spec (`docs/specs/mthds-config-file.md`), so the TypeScript and Python parsers stay byte-identical. `parseDotenv` and `serializeDotenv` are now exported from `src/config/config.ts` and covered by a new unit suite (`tests/unit/config/config-dialect.test.ts`).
...
- **Config file permissions:** `~/.mthds/config` is now written with owner-only permissions (`0600`) instead of relying on the default umask (typically world-readable `0644`), which exposed the stored `MTHDS_API_KEY`. `writeConfigFile` now sets the mode on creation and runs `chmodSync` to tighten pre-existing files.
...
The SDK is physically reorganized so the pure MTHDS Protocol (the interface + wire models) is separated from the runner implementations, mirroring `mthds-python`'s `mthds/protocol/` ⊥ `mthds/runners/` tree. There is no backward compatibility — import paths change.

- **`mthds/client/*` is gone — new module layout.** The protocol interface and wire models now live in `src/protocol/` (`protocol.ts`, `models.ts`, `options.ts`, `pipeline_inputs.ts`, `pipe_output.ts`, `exceptions.ts`, plus the abstract domain hierarchy `concept.ts`, `stuff.ts`, `working_memory.ts`); the API client/runner, run-lifecycle polling, Dict-serialized concretes, and transport errors live in `src/runners/api/` (`client.ts`, `runs.ts`, `models.ts`, `exceptions.ts`); the local CLI runner lives in `src/runners/pipelex/runner.ts`. The public package barrel (`mthds`) re-exports the same surface from the new locations — only deep imports (`mthds/client/*`, never a documented entry point) break. A new `.dependency-cruiser.cjs` rule (run in `make check`) enforces that `protocol/` imports nothing from `runners/`/`cli/`/`agent/`/`config/`.
- **`protocol/` is the full exact mirror of `mthds-python/mthds/protocol/`.** Added the abstract domain shapes the protocol layer carries in python: `ConceptAbstract` (`concept.ts`), `StuffAbstract`/`StuffContentAbstract` (`stuff.ts`), `WorkingMemoryAbstract` (`working_memory.ts`), and `PipeOutputAbstract<TWorkingMemory>` (`pipe_output.ts`) — all exported so consumers import them from `mthds` instead of redefining them. `StuffContentOrData` regains its `StuffContentAbstract` members (mirroring python). The runner-side `DictPipeOutput` is realigned to python's `DictPipeOutputAbstract` shape `{working_memory, pipeline_run_id}` (dropped the never-populated `graph_spec`; the hosted graph artifact rides the durable `/v1/runs/{id}/results` payload, not `pipe_output`).
- **`MTHDS_PROTOCOL_VERSION` is `0.6.0`** (the MTHDS standard version), aligned with `mthds-protocol.openapi.yaml`, the `pipelex` runtime, and `mthds-python` (was a stale `0.1.0`).
- **`RunResult` is split into `RunResultExecute<T>` and `RunResultStart`.** `execute` returns `RunResultExecute<PipeOutputT>{pipeline_run_id, pipe_output}` (output always present — a completed run has output); `start` returns `RunResultStart{pipeline_run_id}` (id only — a started run has no output yet). Both are extension-open via an index signature (`state`, `created_at`, `finished_at`, `main_stuff_name` are implementation extension fields, never named by the SDK). **`MTHDSProtocol` is now generic in the pipe-output type (`MTHDSProtocol<PipeOutputT>`)** — the mechanism that keeps `protocol/` free of the runner-side `DictPipeOutput` concrete (the default `DictRunResultExecute = RunResultExecute<DictPipeOutput>` binding lives in `runners/api/models.ts`). `start`'s return type changing from `RunResult` to `RunResultStart` is an interface-signature change.
- **The API runner IS one client class.** `ApiRunner` is deleted; `MthdsApiClient` now `extends BaseRunner implements Runner` and is the API runner directly — `pipelex-app` uses it as a protocol client, the CLI gets it via `createRunner()`. The client's duplicate `waitForResult`/`startAndWait` are removed in favor of `BaseRunner`'s `waitForResult`/`startAndWaitForResult` composites; the bare-runner blocking-execute fallback (the `/v1/version` handshake) moves onto the client's `startAndWaitForResult` override.
- **Discovery models slimmed to their protocol base, extension-open.** `ModelDeck` base is `{models}` (`aliases`/`waterfalls` are extension fields); `VersionInfo` base is `{protocol_version, runner_version?}` (`implementation`/`implementation_version`/`runtime_version` are extension fields, read via the index signature — the bare-runner `/version` handshake still works); `ValidationReport` is extension-only. Implementations may return more; the extra fields are preserved, never named by the SDK.
...
This release aligns the SDK and CLIs with the MTHDS Protocol (`mthds-protocol.openapi.yaml`): one public API per hostname, every endpoint composed as `{base}/v1/{endpoint}`. There is no backward compatibility (transition by migration, not aliases).

- **Single base URL.** `runnerUrl`/`PIPELEX_RUNNER_URL` + `platformUrl`/`PIPELEX_PLATFORM_URL` are replaced by ONE `base-url` config key / `MTHDS_API_URL` env var — the **host only, no version prefix** (hosted default `https://api.pipelex.com`; self-hosted e.g. `http://localhost:8081`). `PIPELEX_API_KEY` is replaced by `MTHDS_API_KEY`. The "platform follows runner" auto-coupling logic is deleted — there is nothing left to couple. There is NO legacy support at all — the `PIPELEX_*` keys are simply gone (not aliased, not detected, no migration hint); only the canonical `MTHDS_*` keys in `~/.mthds/config` are read. `mthds setup runner api` now asks for one base URL; `mthds-agent runner setup api` takes `--base-url` instead of `--runner-url`/`--platform-url`.
- **`MTHDSProtocol` interface.** `RunnerProtocol` is replaced by `MTHDSProtocol` — the standard's five methods: `execute` (`POST /v1/execute`), `start` (`POST /v1/start`), `validate` (`POST /v1/validate`), `models` (`GET /v1/models`), `version` (`GET /v1/version`). The `Runner` interface now extends it, adding the Pipelex build extensions (`buildInputs`/`buildOutput`/`buildRunner`/`concept`/`pipeSpec` → `/v1/build/*`) and the run-lifecycle feature (`getRunStatus`/`getRunResult`/`waitForResult`/`startAndWaitForResult`) — explicitly a hosted extension, NOT protocol.
- **Client method renames.** `executePipeline` → `execute`; `startPipeline` + `startRun` → one `start` (accepts `pipeline_run_id` — bare-runner-only, the Pipelex Hosted API rejects a client-supplied run id with 422 — plus the generic `extra` extension passthrough: server-specific args merge into the request body and the server that defines them handles them); `getRun` → `getRunStatus` (`GET /v1/runs/{id}/status`); `getResult` → `getRunResult` (`GET /v1/runs/{id}/results`); `pipelex_version` → `version()` (`GET /v1/version`, returning the protocol `VersionInfo` handshake). `hasPlatform()` is deleted — with one base URL, lifecycle availability is decided by the `/v1/version` handshake plus the bare-runner 404 mapping. `ApiRunner` constructor is now `(baseUrl?, apiKey?)`.
- **Wire shape renames (D1).** `pipeline_run_id` (unchanged — D1 revision keeps the id name) and `pipeline_state` → `state` across all request/response models (`RunRequest`, `StartRequest`, `RunResult`, `StartAck`, `RunPublic`, `RunRead`, `RunResults`, `RunResultState`) and the agent CLI JSON envelopes. The stale `dynamic_output_concept_code` field is fixed to the server's actual `dynamic_output_concept_ref` everywhere. Type renames to spec vocabulary: `PipelineRequest` → `RunRequest`, `PipelineExecuteResponse` → `RunResult`, `PipelineStartResponse` → `StartAck`, `ExecutePipelineOptions` → `RunOptions`, `StartRunOptions` → `StartOptions` (now `= StartRequest`), run-artifacts `RunResult` → `RunResults`, `PipelineState` → `RunState` (the protocol's six values). New protocol discovery types: `VersionInfo`, `ValidationReport`, `ModelDeck`/`ModelInfo`/`ModelCategory` (`src/client/protocol-models.ts`).
- **New typed errors.** `RunStillRunningError` — `execute()` maps the protocol's optional `202 + StartAck` degrade to a typed error carrying `runId`, `retryAfterSeconds`, and `location` (defensive: hosted doesn't emit 202 today, the protocol permits it). `RunLifecycleUnavailableError` — a `/v1/runs/*` 404 WITHOUT a structured `code` field (a bare runner serving Starlette's default body) now raises a clear "run polling is a hosted-API extension" error instead of a generic 404, mirroring mthds-python.
- **API `checkModel` deleted.** The `check-model` route never existed on any deployed runner — `ApiRunner.checkModel` had always 404'd. `check-model` stays a LOCAL capability of `PipelexRunner` only (off the shared `Runner` interface); `mthds-agent check-model` on the API runner now errors cleanly pointing at `--runner pipelex`.
- **Protocol-shaped `validate`.** `validate(mthdsContents, allowSignatures?)` returns the protocol `ValidationReport` on success and throws on an invalid bundle (HTTP 422 problem). `mthds-agent validate method`/bare-pipe-code targets error cleanly on the API runner (the protocol validate takes bundle contents, not URLs); `mthds install` validates each method's bundle contents in one call instead of per-pipe `method_url` calls. `models(category?)` takes a single protocol category filter (`llm`, `extract`, `img_gen`, `search`) and returns the protocol `ModelDeck`; `PipelexRunner` maps the local CLI's legacy shape onto it.
- **Minimum server requirement.** Every endpoint now lives under `/v1`: the Pipelex Hosted API requires the `/v1` gateway cutover, and a self-hosted deployment requires an OSS `pipelex-api` image that mounts its API at `/v1` (replacing `/api/v1`). A self-hoster upgrading the SDK first 404s on every call — including the `/v1/version` handshake itself. Upgrade the runner image first.
...
- **Durable run lifecycle on `MthdsApiClient` — `startRun` / `getRun` / `getResult` / `waitForResult`.** Long pipeline runs outlive the API Gateway's 30s ceiling, so the SDK now submits a run on the platform surface (`POST /platform/v1/runs`) and polls a self-healing endpoint by bare `pipeline_run_id` (`GET /platform/v1/runs/by-id/{pipeline_run_id}` and `…/result`) until the run reaches a terminal state. `getResult` maps the platform's poll semantics (HTTP 202 / 200 / 409) to a discriminated `RunResultState`; `waitForResult` honors the server's `Retry-After`, an `AbortSignal`, and a timeout (throwing `RunTimeoutError` without cancelling the run). New `RunFailedError` / `RunTimeoutError` exceptions.
- **Clear timeout error on the blocking `executePipeline`.** When a synchronous run exceeds the Pipelex public API's ~30s gateway ceiling, `MthdsApiClient.executePipeline` now raises `PipelineExecuteTimeoutError` — a message that explains the 30s limit and points to the durable path (`startRun()` + `waitForResult()`, or `mthds-agent run start` + `run poll`) — instead of a cryptic `503 Service Unavailable`. A fast `503` (runner genuinely down) stays an ordinary `ApiResponseError`.
- **`StartRunOptions` mirrors the runner's full ad-hoc input surface.** `pipe_code` is optional (provide `pipe_code` **or** `mthds_contents` — with `mthds_contents` only, the runner resolves the pipe from the bundle's `main_pipe`), plus `output_name`, `output_multiplicity`, and `dynamic_output_concept_ref`, all forwarded through the platform to the runner. Exposed on `mthds-agent run start` as `--output-name` / `--output-multiplicity` / `--dynamic-output`.
- **Agent run commands — `mthds-agent run start` / `run status` / `run result` / `run poll`.** Because all run state lives behind `pipeline_run_id` (DynamoDB + Temporal), an agent can `start` a run, disconnect, and later resume with `status` / `result` / `poll`. `run poll` is Ctrl-C-safe — SIGINT stops waiting without cancelling the run and reports it as resumable by id.

### Changed

- **`Runner` surface reshaped around primitives + composites.** The run lifecycle is now three primitives — `start(options)` (was `startRun`), `getRun(runId)`, `getResult(runId)` — plus two composites provided once by a new `BaseRunner`: `waitForResult(runId)` (poll an already-started run to completion) and `startAndWaitForResult(options)` (start, then wait — the one-call convenience). The redundant `execute()` and the `RunnerProtocol` methods (`executePipeline` / `startPipeline`) are **removed from the `Runner` interface** — the blocking wire calls live on `MthdsApiClient` only. The API runner routes `startAndWaitForResult()` through the durable start+poll path (returning `main_stuff` + `graph_spec`), falling back self-hosted to the runner's blocking `/pipeline/execute` (native `pipe_output`); the local `pipelex` runner supports `startAndWaitForResult` (blocking, in-process) but not the granular durable primitives. The granular durable primitives are **platform-only** — there is no runner fallback, so without a platform URL they fail fast with a clear hosted-only error instead of hitting a runner endpoint that has no run store. The poll/Retry-After/abort/timeout loop is now a single shared `pollUntilResult()` in `client/runs.ts` that both `MthdsApiClient.waitForResult` and `BaseRunner.waitForResult` delegate to, so the behavior can never drift between the wire client and the runner layer.
...
- **`plugin-version.ts` API**. The `PLUGIN_UPDATE_CMD` constant export is replaced by `pluginUpdateCommand(host)`. New exports: `detectHost()`, `readCodexPluginVersion()`, and a `PluginHost` type. `checkPluginVersion()` accepts an optional `host` argument so callers that already detected the host can avoid the duplicate work. Internal callers `bootstrap.ts` (passes an explicit host) and `update-check.ts` (relies on the default-detect path) are unchanged in shape.
...
- **Minimum required pipelex version bumped to `0.26.0`** (was `0.25.0`). Applies to both the `pipelex` and `pipelex-agent` binaries — `mthds-agent` will report `outdated` and emit `UPGRADE_AVAILABLE` from `update-check` when an older version is on PATH.
