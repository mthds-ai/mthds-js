...
- **Client method renames.** `executePipeline` → `execute`; `startPipeline` + `startRun` → one `start` (accepts `method_id` — hosted stored-method extension; `pipeline_run_id` — bare-runner-only, the hosted API rejects a client-supplied run id with 422; and `callback_urls` — HMAC-signed completion webhooks); `getRun` → `getRunStatus` (`GET /v1/runs/{id}/status`); `getResult` → `getRunResult` (`GET /v1/runs/{id}/results`); `pipelex_version` → `version()` (`GET /v1/version`, returning the protocol `VersionInfo` handshake). `hasPlatform()` is deleted — with one base URL, lifecycle availability is decided by the `/v1/version` handshake plus the bare-runner 404 mapping. `ApiRunner` constructor is now `(baseUrl?, apiKey?)`.
