...
- **`codex apply-config` no longer writes a hooks feature flag.** Codex enables hooks out of the box on every supported version: the hook engine (`[features] hooks`, formerly `codex_hooks`) has been Stable and enabled by default since Codex 0.124, plugin-bundled hooks load by default since 0.131, and the old `plugin_hooks` opt-in key was removed (now ignored) in 0.134 — so there is no longer any key to write. `apply-config` now merges just `[sandbox_workspace_write] network_access = true` (still needed for method runs inside the sandbox; the validation hook itself is offline-safe) and continues to sweep any obsolete `~/.codex/hooks.json` entry. Enabling the hook is now nothing more than trusting it on first run (Codex persists trusted hashes under `[hooks.state]`). Requires Codex 0.141.0+, the floor the plugin is tested against — now checked at runtime (see below).
- **Breaking — a hook-disabling `[features]` key is now a hard error, not a warning.** An explicit `hooks = false` (or its deprecated alias `codex_hooks = false`) disables ALL Codex hooks, so `apply-config` reporting `APPLIED` while the mthds hook could never load was a false success — worse, `--check` failed on the same state, so setup flows could never converge. These keys, plus the stale `plugin_hooks` (which disabled plugin-bundled hooks on Codex ≤ 0.133 and is ignored since 0.134), now surface as conflict-style `ConfigError`s in apply, `--check`, `--dry-run`, and `doctor` alike: nothing is written and the user is asked to remove the key by hand — `apply-config` never flips an explicit user choice.
- **New best-effort minimum-Codex-version check, two-tiered.** `codex apply-config` and `doctor` now detect the installed Codex app version (via `codex --version`). Below 0.131 — where plugin-bundled hooks cannot load at all (they were gated behind the since-removed `plugin_hooks` opt-in) — this is a hard `ConfigError` (`CODEX_HOOKS_UNAVAILABLE`) in apply, `--check`, `--dry-run`, and `doctor`: nothing can be written to fix it, so success would present a setup as complete while the hook never loads. Between 0.131 and the supported floor (`MIN_CODEX_VERSION` in `src/agent/codex-version.ts`, currently 0.141.0) hooks work, so it is a `CODEX_VERSION_TOO_OLD` warning that also fails `apply-config --check`. Detection is strictly best-effort: when the `codex` binary is missing or its output unparseable, the check stays silent. On Windows the spawn routes through the shell, since npm installs Codex as a `.cmd` shim that Node cannot execute directly — without it, detection would be silently bypassed on that platform. This closes the gap where `apply-config` reported OK on an old Codex whose bundled hook would silently never load.
