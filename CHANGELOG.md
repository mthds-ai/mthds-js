...
- **(Breaking) The per-pipe build projections answer with a verdict, not a bare payload.** `build/inputs`, `build/output` and `build/runner` each return a discriminated union: the valid arm, or a `CrateInvalidReport` (`is_valid: false` + structured `validation_errors[]`) on a **200**, following `/validate`'s discipline — an unresolvable closure is a produced verdict, not a transport failure. Consumers branch on `is_valid`. The `is_valid: false` arm is produced by the **API runner**; the local `pipelex` runner reaches the same union but never populates it — an unloadable or unresolvable closure makes its CLI exit non-zero and surfaces as a **throw**, so local callers must keep that error handling. `buildInputs` and `buildOutput` were previously typed `Promise<unknown>` and are now fully typed. This does NOT extend to the spec-to-TOML routes `build/concept` and `build/pipe-spec`, which carry no `is_valid` arm and keep their own shapes.
...
- **`allow_signatures` is rejected by the LOCAL runner rather than silently dropped.** `pipelex build runner` exposes no `--allow-signatures` flag, so there was nothing to forward and the flag was being ignored — meaning one `BuildRunnerRequest` meant two different things depending on which runner served it. It now throws, pointing at the API runner.
...
- **The local runner reads the bundle's `domain` / `main_pipe` with the TOML parser, not a regex.** `.mthds` files ARE TOML, but pipe-ref resolution pattern-matched the source and only recognised basic strings — so a perfectly valid `domain = 'smoke'` (a TOML *literal* string) read as declaring no domain at all. Defaulting the selector then failed with "the closure declares no main_pipe", and a bare `--pipe echo` failed with a nonsense "the closure spans domains []". The example in `docs/build-routes.md` was itself single-quoted, i.e. we had documented a case the local runner could not serve. It now parses with `smol-toml` (already a dependency), which also covers quoted keys and comments, so the resolver and the engine agree on what a bundle declares. A closure that genuinely declares no domain says so, and a bundle that will not parse no longer blocks a caller who supplied a qualified `pipe_ref` — the CLI and engine own that diagnostic.
- **A local `buildRunner` no longer discards a valid script over a missing lock file.** It required `structures/codegen.lock` beside the generated `runner.py`, but that lock is written only by pipelex's unreleased codegen engine — so against every *installable* pipelex, a perfectly good `runner.py` was generated and then thrown away with an error. The projection is now a soft postcondition.
...
- **`mthds-agent inputs bundle <file> --content "…"` no longer stamps diagnostics with a file it never submitted.** `--content` overrides the positional target, but the target was still passed as the diagnostics `source`, so the user saw a filename whose bytes were not the ones checked. Now guarded exactly as the sibling `validate bundle` already guarded it.
