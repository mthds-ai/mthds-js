...
- **Typed validation report + structured validation errors on the API runner.** `MthdsApiClient.validate()` now returns a typed `PipelexValidationReport` (the `POST /v1/validate` 200 body: `bundle_blueprint`, `pipe_io_contracts`, `graph_spec`, `validated_pipes` `[{pipe_ref, status}]`, `pending_signatures`, `is_runnable`, plus the route extras `success`/`message`) instead of the opaque `ValidationReport`. `bundle_blueprint`/`pipe_io_contracts`/`graph_spec` stay opaque transport (`Record<string, unknown>` / `unknown`) — their canonical schemas are owned by the runtime and `@pipelex/mthds-ui`. An invalid bundle still throws `ApiResponseError` (throw-on-422 unchanged), now carrying a typed `validationErrors?: ValidationErrorItem[]` parsed from the problem envelope's top-level `validation_errors[]`; it is `undefined` for any non-validation error (auth, transport, a request-shape 422). New exported types: `PipelexValidationReport`, `ValidatedPipeEntry`, `DryRunStatus`, `ValidationErrorItem`, `ValidationErrorCategory`.
- **`validate()` accepts optional per-content names.** `validate(mthdsContents, allowSignatures?, mthdsNames?)` sends an optional `mthds_names[]` parallel to `mthds_contents` — a Pipelex-API extension the server threads onto `blueprint.source`, so cross-file diagnostics resolve the owning file (an unnamed content yields `source: null`). Omitting it sends no `mthds_names` field, identical to before. The protocol `MTHDSProtocol.validate` signature is unchanged (the extra arg is API-runner-only); a caller-supplied `mthds_names`/`mthds_contents` length mismatch surfaces as the server's request-shape `ApiResponseError` (422).
...
- **Agent envelopes for `--runner api` `validate bundle` / `models` change shape with the hosted API's protocol alignment.** These commands re-emit the HTTP body verbatim (`agentSuccess({ ...report })` in `src/agent/commands/api-commands.ts`), so no mthds-js code changed — but what agents see did: `/validate` renames `pipelex_bundle_blueprint` → `bundle_blueprint`, keys `pipe_io_contracts` by namespaced `pipe_ref` (`domain.code`) instead of bare pipe code, and adds `validated_pipes` (`[{pipe_ref, status}]`), `pending_signatures`, and `is_runnable`; valid bundles without a `main_pipe` now return 200 with `graph_spec: null` (previously 422). `/models` gains the protocol's `models` list (`[{name, type}]`), drops `presets`/`success`, keeps category-keyed `aliases`/`waterfalls`, and accepts a single `type` query param only. The `ValidationReport` doc-comment examples and the client test mock now reflect the new field names.
