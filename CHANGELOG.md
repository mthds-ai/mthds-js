...
- **Discriminated `/validate` result union — `/validate` is a 200-diagnostic endpoint.** `MthdsApiClient.validate()` now returns the `PipelexValidationResult` union, discriminated on the mandatory `is_valid` field: `is_valid: true` ⇒ the typed `PipelexValidationReport` (`bundle_blueprint`, `pipe_io_contracts`, `graph_spec`, `validated_pipes` `[{pipe_ref, status}]`, `pending_signatures`, `is_runnable`, plus the route extra `message` and the `mthds_contents` echo); `is_valid: false` ⇒ a `PipelexInvalidReport` (`validation_errors[]`, `pending_signatures`, `is_runnable: false`, `message`, no structural artifacts). `bundle_blueprint`/`pipe_io_contracts`/`graph_spec` stay opaque transport (`Record<string, unknown>` / `unknown`) — their canonical schemas are owned by the runtime and `@pipelex/mthds-ui`. **An invalid bundle is a produced verdict (a 200), NOT a thrown error** — callers pattern-match `is_valid` instead of catching a 422. Only a *no-verdict* condition (a malformed request body, an `mthds_sources` length mismatch, auth, a server fault) is non-2xx and throws `ApiResponseError`. The discriminated union + neutral field names (`is_valid`, `validation_errors`, `pending_signatures`, `is_runnable`) are protocol-level: the protocol layer gains `ValidationResult` / `InvalidValidationReport` / `ValidationError`, the Pipelex extension narrows them to `PipelexValidationResult` / `PipelexInvalidReport` / `ValidationErrorItem`. The closed `ValidationErrorCategory` set gains `"dry_run"` (the graph-level residual diagnostic). The `validation_errors[]` is always non-empty on an invalid verdict (the structured-info invariant is total — even a parse-level failure carries one source-less `blueprint_validation` residual). New exported types: `PipelexInvalidReport`, `PipelexValidationResult`, `ValidationResult`, `InvalidValidationReport`, `ValidationError`, plus the existing `PipelexValidationReport`, `ValidatedPipeEntry`, `DryRunStatus`, `ValidationErrorItem`, `ValidationErrorCategory`. `ApiResponseError.validationErrors` now carries only the **build routes'** 422 diagnostics (`/v1/build/*` still rejects an invalid bundle); the retired `success` wire extra is gone in favor of `is_valid`.
- **`validate()` accepts optional per-content sources (`mthds_sources`).** `validate(mthdsContents, allowSignatures?, mthdsSources?)` sends an optional `mthds_sources[]` parallel to `mthds_contents` — a Pipelex-API extension the server threads onto `blueprint.source`, so cross-file diagnostics resolve the owning file (an unnamed content yields `source: null`). Omitting it sends no `mthds_sources` field. The protocol `MTHDSProtocol.validate` signature is unchanged (the extra arg is API-runner-only); a caller-supplied `mthds_sources`/`mthds_contents` length mismatch surfaces as the server's request-shape `ApiResponseError` (422). (Renamed from the earlier `mthds_names`, matching the stabilized API field.)
- **`VersionInfo.implementation_version` is typed.** The one well-known `VersionInfo` extension is now a named optional field (`implementation_version?: string`) so capability gating reads `version().implementation_version` directly instead of through the untyped index signature. Other extensions (`implementation`, `runtime_version`) still ride the index signature.

### Changed

- **Validate consumers read the verdict off `is_valid`, not a thrown error.** With `/validate` now 200-diagnostic, every command that calls `runner.validate()` (`mthds validate`, `mthds-agent validate`, `mthds install`) checks `report.is_valid === false` and surfaces the structured `validation_errors[]` (the agent path emits a `ValidateBundleError` envelope carrying `is_valid: false` + `validation_errors`), while still catching the no-verdict throw (a local CLI runner raising, or a transport/auth/server fault). Previously an invalid bundle over the API runner threw a 422 the catch handled; that catch no longer fires for content. `agentError` gained optional `is_valid` / `validation_errors` extras to carry the structured failure.
- **Agent envelopes for `--runner api` `validate bundle` / `models` change shape with the hosted API's protocol alignment.** What agents see: `/validate` renames `pipelex_bundle_blueprint` → `bundle_blueprint`, keys `pipe_io_contracts` by namespaced `pipe_ref` (`domain.code`) instead of bare pipe code, adds `validated_pipes` (`[{pipe_ref, status}]`), `pending_signatures`, `is_runnable`, and `is_valid` (the retired `success` extra removed); valid bundles without a `main_pipe` return 200 with `graph_spec: null`, and an invalid bundle returns 200 `is_valid: false` (previously 422). `/models` gains the protocol's `models` list (`[{name, type}]`), drops `presets`/`success`, keeps category-keyed `aliases`/`waterfalls`, and accepts a single `type` query param only.
...
- **Minimum server requirement.** Every endpoint now lives under `/v1`: the hosted API requires the `/v1` gateway cutover, and a self-hosted deployment requires a pipelex-api image that mounts its API at `/v1` (replacing `/api/v1`). A self-hoster upgrading the SDK first 404s on every call — including the `/v1/version` handshake itself. Upgrade the runner image first.
