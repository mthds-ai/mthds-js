...
### Changed
- **(Breaking) Hook-disabling keys are now hard errors** — Setting `[features] hooks = false` (or the deprecated aliases `codex_hooks = false` / `plugin_hooks = false`) now raises a conflict-style `ConfigError` in `apply-config`, `--check`, `--dry-run`, and `doctor`. Previously this was only a warning, which could make setup appear complete while the mthds hook never loaded.
