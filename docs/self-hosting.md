...
The protocol surface (`/v1/execute`, `/v1/start`, `/v1/validate`, `/v1/models`, `/v1/version`) is identical on the Pipelex Hosted API and on a bare runner. Only the hosted extensions differ — e.g. the durable run lifecycle (`/v1/runs/*`) — detectable via the `GET /v1/version` handshake.
...
- **`run start`** → `POST <base>/v1/start` works (fire-and-callback; you may pass your own `pipeline_run_id` — a bare runner accepts it, the Pipelex Hosted API rejects it with 422), but **`run status` / `run result` / `run poll` do not**: the bare runner 404s `/v1/runs/*`, which the SDK surfaces as a clear `RunLifecycleUnavailableError`. The durable poll-by-id lifecycle is a Pipelex Hosted API extension.
