...
- **New best-effort minimum-Codex-version check.** `codex apply-config` and `doctor` now detect the installed Codex app version (via `codex --version`) and emit a `CODEX_VERSION_TOO_OLD` warning — which also fails `apply-config --check` — when it is older than the supported floor (`MIN_CODEX_VERSION` in `src/agent/codex-version.ts`, currently 0.141.0). Detection is strictly best-effort: when the `codex` binary is missing or its output unparseable, the check stays silent. This closes the gap where `apply-config` reported OK on an old Codex whose bundled hook would silently never load.
