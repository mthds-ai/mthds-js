...
- **`MTHDS_STANDARD_VERSION` follows the standard's cut: `1.0.0` → `2.0.0`** (Breaking). The MTHDS specification now defines its own versioning rule (`mthds/docs/spec/versioning.md`) and cut the standard at `2.0.0` to account for the breaking native, manifest, lock and resolution changes it had shipped while the constant sat at `1.0.0`. This package copies that cut. `mthds package init` and `mthds-agent package init` consequently write `mthds_version = ">=2.0.0"` into a new `METHODS.toml`. `MTHDS_PROTOCOL_VERSION` is unchanged at `0.6.0` — the two numbers move independently, and the standard's cut did not touch the HTTP runner contract.
- **A manifest's `mthds_version` is validated as a constraint and evaluated against the standard version.** `validateManifest` previously checked only that the value was a string, so a manifest declaring itself incompatible with this implementation — or carrying an unparseable constraint — installed anyway. It now reports a malformed constraint and an unsatisfied one as errors, which makes the GitHub and local installer resolvers skip the method with a reason. The METHODS.toml parser is deliberately unchanged and still checks shape only: a manifest targeting another standard version stays readable, editable and publishable.
- **`parseConstraint` reads the whole MTHDS constraint grammar** (`src/package/semver.ts`). It was built on npm's `semver.Range`, which reads none of the three ways the standard's grammar differs from npm's — `,` between AND-ed clauses, `==` for exact match, `!=` for exclusion — so a constraint accepted by `isValidVersionConstraint` and by the specification, such as `">=1.0.0, <2.0.0"`, raised `SemVerError` at resolution. The AND-ed clauses are now recombined into one comparator set — not evaluated one range at a time, which would lose npm's prerelease opt-in and make `">=1.0.0-beta.1, <2.0.0"` reject `1.0.0-beta.1` where the equivalent space-separated form accepts it — with each `!=` held aside as an inverted exclusion. `parseConstraint` returns an opaque `VersionConstraint` rather than a `semver.Range`; the module is internal, not part of the published surface, and its own `versionSatisfies` / `selectMinimumVersion` helpers take the new type unchanged. An operator with no operand (`"!="`) is now rejected instead of silently reading as "matches nothing", and a constraint's length and clause count are bounded, since a manifest is fetched from an arbitrary repository and every clause retains a compiled range.
