...
describe("checkPluginVersion (Claude)", () => {
  it("returns ok when plugin version satisfies constraint", () => {
    installPrimaryClaude("0.10.1");
    expect(checkPluginVersion("claude")).toEqual({ s: "ok", v: "0.10.1" });
  });
      ...
      JSON.stringify({
        ...
        plugins: {
          [PLUGIN_KEYS[0]]: [
            ...
            { scope: "user", version: "0.10.1", installPath: "/tmp" },
          ],
        },
      })
    );
  ...
  it("prefers prod key when both prod and dev keys exist", () => {
    fsState.files.set(
      ...
      JSON.stringify({
        version: 2,
        plugins: {
          [PLUGIN_KEYS[0]]: [{ scope: "user", version: "0.10.1", installPath: "/tmp" }],
...
describe("checkPluginVersion (Codex)", () => {
  it("returns ok when a satisfying version is installed", () => {
    installCodexPlugin("mthds", ["0.10.1"]);
    expect(checkPluginVersion("codex")).toEqual({ s: "ok", v: "0.10.1" });
  });

  it("picks the highest version directory when multiple are installed", () => {
    installCodexPlugin("mthds", ["0.9.0", "0.10.1", "0.8.3"]);
    expect(checkPluginVersion("codex")).toEqual({ s: "ok", v: "0.10.1" });
  });
  ...
  it("returns null when 'local' dev sentinel is present", () => {
    installCodexPlugin("mthds", ["local", "0.10.1"]);
  ...
  it("ignores dotfile entries when picking a version", () => {
    fsState.dirs.set(codexPluginDir("mthds"), [".DS_Store", "0.10.1"]);
    fsState.dirs.set(codexVersionDir("mthds", "0.10.1"), [".codex-plugin"]);
    fsState.dirs.set(
      join(codexVersionDir("mthds", "0.10.1"), ".codex-plugin"),
      ["plugin.json"]
    );
    fsState.files.set(
      codexManifestPath("mthds", "0.10.1"),
  ...
  it("prefers the manifest version over the directory name", () => {
    installCodexPlugin("mthds", ["0.10.1"], { manifestVersion: "0.10.5" });
  ...
  it("falls back to the directory name when the manifest is unreadable", () => {
    ...
    fsState.dirs.set(codexPluginDir("mthds"), ["0.10.1"]);
    fsState.dirs.set(codexVersionDir("mthds", "0.10.1"), [".codex-plugin"]);
    expect(checkPluginVersion("codex")).toEqual({ s: "ok", v: "0.10.1" });
  });
  ...
  it("respects $CODEX_HOME when set", () => {
    ...
    installCodexPlugin("mthds", ["0.10.1"], { codexHome: customHome });
    expect(checkPluginVersion("codex")).toEqual({ s: "ok", v: "0.10.1" });
  });
});
