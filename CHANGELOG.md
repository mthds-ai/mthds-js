...
- **`InputFormTopLevelField` now encodes the presence pairing as a discriminated union.** The top-level input-form field type is a union discriminated on `required`, so the spec page's derivations are the type rather than prose beside it — the idiom `PipeInputContract` already uses. `required` restates the marker exactly as the page derives it (`required: true` pairs only with `presence: "plain" | "force"`, written `Exclude<PresenceMarker, "optional">` to stay tied to the marker vocabulary; `required: false` only with `"optional"`), and the optional arm pins `gating: false`, the gating table's `Concept?` row. A literal like `{required: true, presence: "optional"}` — which `mthds-python` rejects at the parse with `validate_required_restates_presence` — now fails to compile here, closing the two-client divergence, and narrowing works both ways for consumers (`presence === "optional"` narrows `required` to `false` and `gating` to `false`). The derivation a type cannot state — a variable list is required yet never gates — stays a documented producer obligation. `input-form-shapes.test.ts` grows the matching positive `force` case, three rejected literals, and `Extract`-based type assertions. breaking for a consumer that assembled a top-level field from computed booleans: branch once on optionality and write each arm as a literal.

- **`/contract-check` files its findings as workspace ledger items instead of a report file.** The skill no longer offers to write `wip/contract-check-<baseline>-to-<target>.md`. Each discrepancy and unmatched addition becomes one ledger item owned by the repo that has to act on it — `workspace` for a `docs/specs/` edit, `mthds-js` for a code fix, the implementing repo for anything else, and a `decision` owned by `workspace` when which side moved is genuinely undetermined, which is how the skill keeps its stance of detecting drift without assigning blame. The check runs before every release against a moving baseline, so it now looks for what a previous run already filed (every item carries `--ref skill:contract-check`) and records a fresh sighting on the existing item rather than filing a duplicate. Findings owned by `mthds-js` are offered as a fix-now first: the ledger is for work crossing a boundary, never a way to defer work that belongs in the session that found it. `.claude/skills/release/SKILL.md` records what this means for a release — the findings survive it either way, already owned and tracked, and a finding fixed during the release lands in its own commit before the release commit.
