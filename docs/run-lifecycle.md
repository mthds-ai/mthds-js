...
- **Run-lifecycle extension** (`GET /v1/runs/{run_id}/status`, `GET /v1/runs/{run_id}/results`) — the durable polling surface. It is a **hosted-API feature, NOT part of the protocol**. A bare [pipelex-api](https://github.com/Pipelex/pipelex-api) runner has no run store and 404s these routes, which the SDK translates into a clear `RunLifecycleUnavailableError`.
...
| Method | Path | Returns |
|---|---|---|
| `POST` | `/v1/start` | `202 StartAck` — the authoritative `run_id` (executes asynchronously) |
| `GET` | `/v1/runs/{run_id}/status` | `RunRead` — status, self-healing (`degraded` flag + `Retry-After` when Temporal is unreachable) |
| `GET` | `/v1/runs/{run_id}/results` | `202` still running · `200` result · `409` terminal failure |
...
Hosted-only `start` extras: `method_id` runs a stored method from the active org's catalog. A client-supplied `run_id` is bare-runner-only — the hosted API rejects it with 422 (the `StartAck.run_id` is always authoritative).
...
const ack = await client.start({
  ...
  // protocol async extras: run_id (bare runners only), callback_urls
});

// Poll to completion
try {
  const result = await client.waitForResult(ack.run_id, {
...
The blocking `execute()` path may also throw `RunStillRunningError`: the protocol permits an implementation to degrade a synchronous `/execute` into `202 + StartAck` when it cannot hold the connection open. The error carries the `run_id`, the `Retry-After` hint, and the `Location` header — resume by id.
...
# → { "run_id": "…", "state": "STARTED", "created_at": "…" }
...
```json
{ "state": "running", "run_id": "…", "resumable": true,
