...
vi.mock("../../../src/cli/commands/share.js", () => ({
  buildShareUrl: vi.fn().mockReturnValue("https://twitter.com/intent/tweet?text=test"),
}));
...
const mockedBuildShareUrl = vi.mocked(buildShareUrl);
...
describe("agentPublish", () => {
  ...
  it("includes share_url when --share flag is set", async () => {
    mockedParseAddress.mockReturnValue({ org: "mthds-ai", repo: "contract-analysis", subpath: null });
    mockedResolveFromGitHub.mockResolvedValue(fakeResolved);
    mockedBuildShareUrl.mockReturnValue("https://twitter.com/intent/tweet?text=hello");

    await agentPublish("mthds-ai/contract-analysis", { share: true });

    expect(mockedBuildShareUrl).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(mockedAgentSuccess).toHaveBeenCalledWith(
      expect.objectContaining({
        share_url: "https://twitter.com/intent/tweet?text=hello",
      })
    );
  });

  it("does not include share_url when --share flag is not set", async () => {
    mockedParseAddress.mockReturnValue({ org: "mthds-ai", repo: "contract-analysis", subpath: null });
    mockedResolveFromGitHub.mockResolvedValue(fakeResolved);
    ...
    expect(successCall).not.toHaveProperty("share_url");
  });
