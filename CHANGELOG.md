...
- **Single base URL.** `runnerUrl`/`PIPELEX_RUNNER_URL` + `platformUrl`/`PIPELEX_PLATFORM_URL` are replaced by ONE `base-url` config key / `MTHDS_API_URL` env var — the **host only, no version prefix** (hosted default `https://api.pipelex.com`; self-hosted e.g. `http://localhost:8081`). `PIPELEX_API_KEY` is replaced by `MTHDS_API_KEY`. The "platform follows runner" auto-coupling logic is deleted — there is nothing left to couple. A leftover legacy key (env or `~/.mthds/config`) makes the API runner fail fast with a one-line migration hint (`mthds config set base-url …` / `mthds config set api-key …`). `mthds setup runner api` now asks for one base URL; `mthds-agent runner setup api` takes `--base-url` instead of `--runner-url`/`--platform-url`.
