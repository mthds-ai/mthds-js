...
- **`parseConstraint` reads the whole MTHDS constraint grammar** (`src/package/semver.ts`). It was built on npm's `semver.Range`, which reads none of the three ways the standard's grammar differs from npm's — `,` between AND-ed clauses, `==` for exact match, `!=` for exclusion — so a constraint accepted by `isValidVersionConstraint` and by the specification, such as `">=1.0.0, <2.0.0"`, raised `SemVerError` at resolution. Constraints are now evaluated clause by clause, and `parseConstraint` returns an opaque `VersionConstraint` rather than a `semver.Range` — an internal module, not part of the published surface, and its own `versionSatisfies` / `selectMinimumVersion` helpers take the new type unchanged. This also repairs dependency resolution, which shared the same parser.
