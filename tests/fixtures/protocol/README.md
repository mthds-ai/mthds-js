...
The corpus is **contract-first**: it landed before either projection, so each is written against a stated expectation instead of the expectation being back-filled from whatever the first implementation happened to emit. `tests/unit/protocol/inputs-template-projection.test.ts` runs its four jobs — byte parity (skip-gated until `src/protocol/inputs_template.ts` exists, `L-260829-f50e2b`), kind coverage against the whole closed `FIELD_KINDS` vocabulary, file-set completeness, and the divergence lapse check.
